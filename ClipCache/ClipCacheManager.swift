...
class ClipCacheManager: ObservableObject {
    ...
    @Published var clearCacheShortcutKey: UInt32 = 0x0F // R key
    ...
    @Published var captureText: Bool = false
    ...
    private let defaultClearCacheShortcutKey: UInt32 = 0x0F // R key
    ...
    private let defaultCaptureImages: Bool = true
    private let defaultCaptureText: Bool = false
    ...
    func checkAndPromptForPermissionsIfNeeded() {
        ...
        let hasShownPrompt = UserDefaults.standard.bool(forKey: hasShownFirstRunPermissionPromptKey)
        
        // Check if accessibility permissions are granted
        let hasPermissions = AXIsProcessTrusted()
        
        // If first run and no permissions, open System Settings
        if !hasShownPrompt && !hasPermissions {
    ...
    func batchPasteGroup() {
        guard !clipboardCache.isEmpty || !textCache.isEmpty else { return }
               ...
               !images.isEmpty {
                ...
                if let firstImage = images.first,
                   ...
                   imageHash == lastPastedImageHash {
                    ...
                    if clipboardCache.isEmpty && textCache.isEmpty {
                    ...
                    imageCount = clipboardCache.count + textCache.count
                    lastCaptureDate = now
                }
            }
        }
